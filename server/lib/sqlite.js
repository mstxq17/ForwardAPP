...
var DB = DB || {};
var dataPath = config.appDir + 'data/';

DB.easySqlite = function(file){
    var filePath =  dataPath + file + '.db';
...
DB.easySqlite.prototype.initTable = function(){
     ...
     * 删除表内容
     */
    DB.db.backup(`${dataPath}backup-${Date.now()}.db`)
      .then(() => {
    ...
    /**
     * 域名表 domain
     * column: username Domain proxy Arecord status
     */
    var createDomainTableSql = "create table if not exists domain(`id` integer primary key autoincrement,`username` varchar(255) not null,`Domain` varchar(255) not null,`proxy` varchar(255) not null, `Arecord` varchar(255) not null,`status` varchar(255) not null);";
